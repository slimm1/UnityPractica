...
public class HellixController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        //luego rotarlo hacia la izquierda o hacia la derecharotacionIncial = transform.localEulerAngles;
        //Calculamos la distancia entre el HellixTop y el HellixGoal añadiéndole 0.1 de margen
        distanciaHelix = topTransform.localPosition.y - (goalTransform.localPosition.y +
       .1f);
    ...
    public void CargarStage(int numeroStage)
    {
        ...
        FindObjectOfType<BolaController>().GetComponent<Renderer>().material.color =
        allStages[numeroStage].colorBolaStage;
        ...
        for (int i = 0; i < stage.niveles.Count; i++)
        {
            ...
            while (partesDeshabilitadas.Count < partesToDisable)
            {
               ...
               nivel.transform.GetChild(Random.Range(0, nivel.transform.childCount)).gameObject
               ;
            ...
            //vamos ahora a cambiar el color de todas las partes habilitadas de los discos, para ello hacemos un foreach para ir cambiando el color de todos los componentesdel nivel
            foreach (Transform t in nivel.transform)
            {
                t.GetComponent<Renderer>().material.color =
               allStages[numeroStage].colorParteNivelStage;
            ...
            while (deathParts.Count < stage.niveles[i].numeroDeathPart)
            {
                ...
                GameObject randomPart = partesHabilitadas[(Random.Range(0,
               partesHabilitadas.Count))];
